...
class StewartEnv(robot_gazebo_env.RobotGazeboEnv):
    ...
    def __init__(self):
        ...
        self.init_force_msg = Float32MultiArray()
        self.init_joint_msg = Float32MultiArray()

        self.end_effector_pose_cmd = Twist()
        self.pid_values = Vector3()

        self._joints_force_cmd_pub = rospy.Publisher('/stewart/legs_force_cmd',  Float32MultiArray, queue_size=10) # set leg forces directly.
    ...
    def set_force_joints(self, applied_leg_forces):
        """
        Sets the applied effort for the specified joints.
        This force will persist across time steps.
        """
        self.init_force_msg.data = applied_leg_forces

        try:
            self._joints_force_cmd_pub.publish(self.init_force_msg)
            result = True
        except Exception as ex:
            print(ex)
            result = False
