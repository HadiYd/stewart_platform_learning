...
def main(args=None):
    ...
    X,Y, _, _ = rollout(env=env, pilco=None, verbose=False, random=True, timesteps=40, render=False)
    for i in range(1,5):
        X_, Y_, _, _ = rollout(env=env, pilco=None, verbose=False, random=True,  timesteps=1000, render=False)
        X = np.vstack((X, X_))
        Y = np.vstack((Y, Y_))
    ...
    for rollouts in range(3):
        ...
        X_new, Y_new, _, _ = rollout(env=env, pilco=pilco, timesteps=100, render=False)
        # Update dataset
